...
func InstallPluginsFromLocalSource(pluginName, version string, target cliv1alpha1.Target, localPath string, installTestPlugin bool) error {
	...
	if len(matchedPlugins) == 1 {
		return installOrUpgradePlugin(&matchedPlugins[0], PluginVersion(&matchedPlugins[0], version), installTestPlugin)
	}

	for i := range matchedPlugins {
		if matchedPlugins[i].Target == "" || matchedPlugins[i].Target == target {
			err = installOrUpgradePlugin(&matchedPlugins[i], PluginVersion(&matchedPlugins[i], version), installTestPlugin)
			if err != nil {
				errList = append(errList, err)
			}
		}
...
func PluginVersion(discovered *plugin.Discovered, version string) string {
	if discovered == nil || version != "" {
		return version
	}
	return discovered.RecommendedVersion
}
