...
func TestGetClientConfigV2Node(t *testing.T) {
	// Setup config data
	f, err := os.CreateTemp("", "tanzu_configv2")
	assert.Nil(t, err)
	err = os.WriteFile(f.Name(), []byte(""), 0644)
	assert.Nil(t, err)
	err = os.Setenv("TANZU_CONFIG_V2", f.Name())
	...
	//Action
	node, err := getClientConfigV2Node()
...
func TestClientConfigV2NodeUpdateInParallel(t *testing.T) {
	addContext := func(mcName string) error {
		_, err := getClientConfigV2Node()
		if err != nil {
			return err
		}
